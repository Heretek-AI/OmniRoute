...
const previousPluginsDir = process.env.OMNIROUTE_PLUGINS_DIR;
...
test.after(() => {
  ...
  if (previousPluginsDir === undefined) delete process.env.OMNIROUTE_PLUGINS_DIR;
  else process.env.OMNIROUTE_PLUGINS_DIR = previousPluginsDir;
});
