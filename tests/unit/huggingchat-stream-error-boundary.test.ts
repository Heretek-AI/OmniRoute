...
const testRoot = mkdtempSync(join(tmpdir(), "omniroute-huggingchat-error-boundary-"));
...
test("HuggingChat partial failures reach stream finalization, persistence, and fallback", async () => {
  const model = "test/huggingchat-model";
  const provider = "huggingchat";
  const connectionId = "huggingchat-stream-error-boundary";
